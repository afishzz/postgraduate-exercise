    ...
    <ul class="container">
      ...
      <li v-for="(item, index) in items" :key="item.id" :class="item.name">
        ...
        <input
          ...
          @blur="restoreStyle(index)"
        />
        <!-- 其他输入框 -->
        <input
          v-if="item.name != 'gender' && item.name != 'goal'"
          type="text"
          v-model="item.val"
          :class="{ active: activeIndex == index ? true : false }"
          placeholder="未设置"
...
export default {
  ...
  methods: {
      ...
      getUserAllInfo({
        ...
        .then((res) => {
          if (res.code == 0) {
            ...
            this.items[2].val =
              res.data.school == null || res.data.school == ""
                ? ""
                : res.data.school;
            this.items[3].val =
              res.data.motto == null || res.data.motto == ""
                ? ""
                : res.data.motto;
    ...
    saveUserInfo() {
      ...
      this.items[4].val =
        this.items[4].val.length == 0 ? 0 : parseInt(this.items[4].val);
      saveUserAllInfo({
        openID: getApp().globalData.openID,
