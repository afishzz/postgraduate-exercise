<template>
  <view class="result-wrapper">
    ...
    <view class="answer-block">
      <view
        class="ques-block"
        v-for="(item, index) in quesArr"
        :key="index"
        @click="naviToQuesDetail(index)"
      >
        <text>{{ index + 1 }}</text>
        <view class="is-wrong-block">
          <text class="isWrong" v-if="quesArr[index] === 0">未做答</text>
          <i class="iconfont isWrong right" v-else-if="quesArr[index] === 1"
            >&#xe60a;</i
          >
...
.result-wrapper {
  ...
  .wrapper {
    ...
    display: flex;
    border-radius: 25rpx;
    box-shadow: 0rpx 5rpx 10rpx 10rpx rgba(1, 1, 1, 0.05);
  ...
  .answer-block {
    ...
    box-shadow: 3rpx 4rpx 10rpx 5rpx rgba(0, 0, 0, 0.1);
