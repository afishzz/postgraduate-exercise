  ...
  <view class="wrapper">
    <view class="smart-test-wrapper" v-if="questionReady">
      ...
      <view class="change-ques">
        <button
          :class="[
            isConfirm && index === questions.length - 1 ? '' : 'pre-btn',
            'btn',
          ]"
          @click="changeToPre"
          v-show="index > 0"
        ...
        <button
          :class="[index === 0 ? '' : 'next-btn', 'btn']"
          @click="changeToNext"
          v-show="index < questions.length - 1"
        >
          下一题
        </button>
        <!-- 确认按钮 -->
        <button
          :class="['next-btn', 'btn']"
          @click="confirmSubmit"
          v-show="!isConfirm && index === questions.length - 1"
        >
          完成
        </button>
      </view>
    ...
    <view
      class="fullopacity fullbg"
      v-if="showDetail"
      @click="hideAnswerSheet"
    ></view>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    changeOption(index) {
      ...
      this.bgColors.splice(
        index,
        1,
        this.bgColors[index] ? "" : "choose-bg-color"
      );
    ...
    changeToNext() {
      ...
      this.resetData();
      this.index =
        this.index < this.questions.length - 1 ? ++this.index : this.index;
      this.setOptions();
      if (this.isConfirm) {
        // 做完了
        this.confirmAnswer();
    ...
    confirmAnswer(firstDone = false) {
      ...
      // 判断正误
      this.isCorrect =
        this.choosedAnswers[this.index].sort().join("") === correctAnswer
          ? true
          : false;
      ...
      // 错误答案
      this.choosedAnswers[this.index].forEach((val) => {
        style[ans.indexOf(val)] = correctAnswer.includes(val)
    ...
    setQuestionsDone() {
      ...
      const isWrong = this.questions.map(
        (val, index) =>
          val.answer !== this.choosedAnswers[index].sort().join("")
      );
