<template>
  <view class="question-wrapper" v-if="questionReady">
    ...
    <!--进度条-->
    <progress :currentIndex="index + 1" :totalNum="questions.length"></progress>
    <!--单选 正确率-->
    <option-right
    ...
    <view class="change-ques">
      <button
        :class="[index === questions.length - 1 ? '' : 'pre-btn', 'btn']"
        @click="changeToPre"
        v-show="isConfirm && index !== 0"
      >
        上一题
      </button>
      <button
...
export default {
  ...
  methods: {
    ...
    confirmAnswer(isDone = false) {
      ...
      // 判断正误
      this.isCorrect =
        this.userAnswer.sort().join("") === correctAnswer ? true : false;
      // 设置选项样式
